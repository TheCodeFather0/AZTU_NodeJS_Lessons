...
function fetchAsyncQuestionProperty(question, prop, answers) {
    ...
    return from(runAsync(value)(answers).then((value) => {
        question[prop] = value;
        return question;
    }));
}
 ...
 */
export default class PromptsRunner extends Base {
    prompts;
    answers = {};
    process;
    opt;
    ...
    processQuestion(question) {
        ...
        return defer(() => {
            ...
            return obs.pipe(concatMap(this.setDefaultType.bind(this)), concatMap(this.filterIfRunnable.bind(this)), concatMap((question) => fetchAsyncQuestionProperty(question, 'message', this.answers)), concatMap((question) => 
            // @ts-expect-error question type is too loose
            fetchAsyncQuestionProperty(question, 'default', this.answers)), concatMap((question) => 
            // @ts-expect-error question type is too loose
            fetchAsyncQuestionProperty(question, 'choices', this.answers)), concatMap((question) => {
                // @ts-expect-error question type is too loose
